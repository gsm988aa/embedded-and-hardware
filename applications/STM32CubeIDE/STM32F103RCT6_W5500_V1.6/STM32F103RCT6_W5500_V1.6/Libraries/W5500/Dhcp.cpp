...
int DhcpClass::request_DHCP_lease(){
    ...
    _dhcpTransactionId = /*random(1UL, 2000UL);*/rand() % (2000UL ) + 1UL;
    ...
    while(_dhcp_state != STATE_DHCP_LEASED)
    {
        if(_dhcp_state == STATE_DHCP_START)
        {
            ...
            send_DHCP_MESSAGE(DHCP_DISCOVER, ((HAL_GetTick() - startTime) / 1000));
            _dhcp_state = STATE_DHCP_DISCOVER;
        }
        else if(_dhcp_state == STATE_DHCP_REREQUEST){
            _dhcpTransactionId++;
            send_DHCP_MESSAGE(DHCP_REQUEST, ((HAL_GetTick() - startTime)/1000));
        ...
        if(result != 1 && ((HAL_GetTick() - startTime) > _timeout))
            break;
    }
...
uint8_t DhcpClass::parseDHCPResponse(unsigned long responseTimeout, uint32_t& transactionId)
{
    ...
    {
        if((HAL_GetTick() - startTime) > responseTimeout)
        {
